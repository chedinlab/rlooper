...
class RLoopGraph:
    @classmethod
    def __create_pdf(cls, file_name, probability, energy, start_loci, end_loci):
    ...
    @classmethod
    def plot(cls, probability_file, energy_file, out_filename_prefix=''):
            ...
            while p_line != '':
                ...
                if header1_pattern.match(p_line):
                    if len(probability) > 0:
                        ...
                        cls.__create_pdf(out_filename_prefix + seq_name + '.pdf', probability, energy, start_loci,
                                         end_loci)
        ...
        if len(probability) > 0:
            if len(energy) < 1:
                raise AssertionError('Energy info for ' + seq_name + ' not found')
            cls.__create_pdf(out_filename_prefix + seq_name + '.pdf', probability, energy, start_loci, end_loci)


if __name__ == '__main__':
    args = vars(RLoopGraph.get_args())
    RLoopGraph.plot(args.get('input_file_probability'), args.get('input_file_energy'),
                    args.get('output_file_prefix', ''))
